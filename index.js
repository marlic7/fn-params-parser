...
module.exports = function (args, spec) {
    let outParams = {};
        ...
        spec.forEach(function(v) {
	        if(_.has(args[0], v.name)) {
                if(checkTypeMatch(args[0][v.name], v.type)) {
                    ...
                    throw new Error(`No proper ${v.name} parameter data type. Expected type is: ${getTypes(v.type)}!`);
                }
...
function getTypes(type) {
    ...
    return types.map(function(i) {
        return (typeof i === 'function' ? i.name : i);
    }).join('|');
}
